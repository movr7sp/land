...
import { Location } from "@/components/ui/permanent/text";
...
import { cormorantGaramond, sourceSerif4 } from "@/components/ui/permanent/font";
...
export function MainPortrait() {
    ...
    return (
    <div className={`landscape:hidden h-screen overflow-y-scroll snap-y snap-mandatory scroll-smooth lg:hidden `} ref={containerRef}>
        ...
        <div className={`flex flex-col lg:flex-row h-screen w-full snap-start opacity-0`} ref={(el) => {if (el && !sectionsRef.current.includes(el)) {sectionsRef.current.push(el);}}}>
            <div className={`flex flex-col bg-white justify-center items-center`}>
                <Image src={m3} className={`mask-b-from-40% mask-b-to-100% md:mask-r-from-95% md:mask-l-from-95% `} alt={`m3`}></Image>
            </div>
